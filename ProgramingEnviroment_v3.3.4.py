# filename: ProgramingEnviroment_v3.3.4.py
# --- ここから ---
...
class ProgramingEnviromentApp:
    ...
    def get_project_name(self, file_name):
        # プロジェクト名は最初のスペース、ハイフンまで（アンダースコアは区切りとして扱わないように改修）
        clean_name = re.split(r'[ \-]', file_name)[0]
    ...
    def run_pipeline(self):
        ...
        # サブプロセス判定
        is_sub = "sub_process: True" in code_content
        prog_name = self.get_project_name(file_name)
        
        # ターゲットディレクトリの決定
        base_target_dir = os.path.abspath(os.path.join(self.config["base_dir"], prog_name))
        if is_sub:
            target_dir = os.path.join(base_target_dir, "processors")
            self.log(f"Sub-process detected. Targeting: /processors folder", "sync")
        else:
        ...
        try:
            ...
            self.log(f"Saved: {file_name} to {target_dir}", "success")
            
            # サブプロセスの場合はGit追加のみ行い、ビルド(EXE化)をスキップ
            self.execute_git(target_dir, file_name)
            
            if not is_sub:
                self.execute_build(target_dir, file_name)
                self.log("Pipeline finished (EXE Build complete).", "success")
            else:
                self.log("Pipeline finished (Sub-process saved, Build skipped).", "success")
    ...
    def execute_git(self, target_dir, file_name):
        try:
            repo_dir = target_dir
            # processorsフォルダ内の場合は親フォルダがリポジトリルートか確認
            if "processors" in target_dir:
                repo_dir = os.path.dirname(target_dir)

            repo = git.Repo(repo_dir) if os.path.exists(os.path.join(repo_dir, ".git")) else git.Repo.init(repo_dir)
            
            # 相対パスでの追加を確実にする
            rel_path = os.path.relpath(os.path.join(target_dir, file_name), repo_dir)
            repo.index.add([rel_path])
            repo.index.commit(f"Auto-update: {file_name}")
            
            if 'origin' in [r.name for r in repo.remotes]:
                repo.remote(name='origin').push()
        except Exception as e: 
            self.log(f"Git Note: {e}", "info")

    def execute_build(self, target_dir, file_name):
        dist_path = os.path.abspath(os.path.join(target_dir, "dist"))
        self.log(f"Building EXE for {file_name}...", "info")
