# filename: ProgramingEnviroment_v3.1.5.py
...
class ProgramingEnviromentApp:
    ...
    def run_gemini_monitor(self):
        try:
            # EXE化対策: Playwrightに本体ブラウザの場所を教え込む
        ...
        with sync_playwright() as p:
            try:
                self.log(f"Pointing browser path to: {pw_browser_path}", "sync")
                # ブラウザ起動 (サンドボックス解除オプション付き)
                ...
                self.log("Browser window created. Navigating to Gemini...", "sync")
                page.goto("https://gemini.google.com/app", wait_until="domcontentloaded")
                self.log("Sync Active! Chat monitoring started.", "success")
                            ...
                            for block in reversed(blocks):
                                ...
                                if "# filename:" in content and "# ---ここまで---" in content:
                                    if content != self.last_sync_content:
                                        ...
                                        self.root.after(0, lambda c=content: self.update_code_from_sync(c))
                                    break
                    except: pass
                    time.sleep(2)
                browser.close()
            except Exception as e:
                self.log(f"CRITICAL SYNC ERROR: {str(e)}", "error")
                self.sync_active = False
                self.root.after(0, lambda: self.sync_btn.config(text="🌐 Start Sync", bg="#34a853"))
    ...
    def update_code_from_sync(self, content):
        ...
        self.log("Sync Success: Code imported to editor.", "success")
        self.root.bell()
