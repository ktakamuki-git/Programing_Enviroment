# filename: ProgramingEnviroment_v3.3.5.py
# --- ここから ---
...
class ProgramingEnviromentApp:
    ...
    def run_gemini_monitor(self):
        try:
            ...
            with sync_playwright() as p:
                self.browser_context = p.chromium.launch_persistent_context(user_data_dir=session_dir, headless=False)
    ...
    def cleanup_old_exes(self, target_dir=None, current_file=None):
        # target_dir未指定時はツール自身のフォルダを整理
        base = target_dir if target_dir else os.getcwd()
        dist = os.path.join(base, "dist") if target_dir else base
        if not os.path.exists(dist): return
        
        ver_dir = os.path.join(dist, "Versions")
        os.makedirs(ver_dir, exist_ok=True)
        
        current_exe = current_file.replace(".py", ".exe") if current_file else f"{APP_NAME}_v{VERSION}.exe"
        
        for f in os.listdir(dist):
            if f.endswith(".exe") and f != current_exe:
                try: shutil.move(os.path.join(dist, f), os.path.join(ver_dir, f))
                except: pass
