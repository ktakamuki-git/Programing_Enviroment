# filename: ProgramingEnviroment_v3.3.1.py
...
class ProgramingEnviromentApp:
    ...
    def run_gemini_monitor(self):
        ...
        with sync_playwright() as p:
            try:
                self.log("Launching browser with session...", "sync")
                ...
                self.log("Sync Active! Session restored.", "success")
    ...
    def run_pipeline(self):
        ...
        if len(lines) > 1:
            sub_match = re.search(r'sub_process:\s*(True|False)', lines[1], re.IGNORECASE)
            parent_match = re.search(r'main_parent:\s*([\w\.\- ]+)', lines[1], re.IGNORECASE)
            if sub_match and sub_match.group(1).lower() == "true":
                is_sub = True
                if parent_match:
                    main_parent = parent_match.group(1).strip()

        if is_sub and main_parent:
            prog_name = main_parent
        else:
        ...
        try:
            ...
            if is_sub:
                self.log(f"Sub-process saved to {full_path}. Build skipped.", "success")
                messagebox.showinfo("Success", f"Sub-process saved to {prog_name} folder.")
            else:
    ...
    def organize_old_files(self, target_dir, current_file_name):
        """
        修正版：
        現在保存しようとしているファイル名(例: MyTool_v3.2.py)の
        ベース部分(MyTool)が一致し、かつ自分自身ではないファイルのみを old/ に移動する。
        """
        old_dir = os.path.join(target_dir, "old")
        if not os.path.exists(old_dir): os.makedirs(old_dir)
        
        # バージョン表記( _vX.X.X )を無視したベース名を取得
        base_name_match = re.split(r'_v\d+', os.path.splitext(current_file_name)[0])
        if not base_name_match: return
        target_prefix = base_name_match[0]

        for f in os.listdir(target_dir):
            if f.endswith((".py", ".spec")) and f != current_file_name:
                if f in ["old", "dist", "build", ".git", ".sync_session"]: continue
                
                # 保存中ファイルのベース名と、対象ファイルのベース名が一致するか
                f_base = re.split(r'_v\d+', os.path.splitext(f)[0])[0]
                if f_base == target_prefix:
                    self.log(f"Moving old version to old/: {f}")
                    try:
                        dest = os.path.join(old_dir, f)
                        if os.path.exists(dest): os.remove(dest) # 重複回避
                        shutil.move(os.path.join(target_dir, f), dest)
                    except: pass
