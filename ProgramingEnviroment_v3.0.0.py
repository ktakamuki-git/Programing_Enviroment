# filename: ProgramingEnviroment_v3.0.0.py
...
class ProgramingEnviromentApp:
    ...
    def setup_ui(self):
        ...
        # --- Sync Toggle Button ---
        self.sync_btn = tk.Button(header, text="🌐 Start Gemini Sync", command=self.toggle_sync, bg="#34a853", fg="white", bd=0, padx=15, font=("Segoe UI", 9, "bold"))
    ...
    # --- Playwright Sync Logic ---
    def toggle_sync(self):
        if not self.sync_active:
            self.sync_active = True
            self.sync_btn.config(text="🛑 Stop Gemini Sync", bg="#ea4335")
            self.log("Starting Gemini Sync Engine...", "sync")
            threading.Thread(target=self.run_gemini_monitor, daemon=True).start()
        else:
            self.sync_active = False
            self.sync_btn.config(text="🌐 Start Gemini Sync", bg="#34a853")
    ...
    def run_gemini_monitor(self):
        try:
            ...
            self.log("Playwright not found. Installing...", "error")
            subprocess.run(["py", f"-{TARGET_PYTHON}", "-m", "pip", "install", "playwright"], shell=True)
            self.log("Please run 'playwright install chromium' in your terminal and restart.", "error")
            self.sync_active = False
            return

        with sync_playwright() as p:
            browser = p.chromium.launch(headless=False) # ログインが必要なためヘッドフルで起動
            page = browser.new_page()
            page.goto("https://gemini.google.com/app")
            self.log("Browser launched. Please login to Gemini if needed.", "sync")
            ...
            while self.sync_active:
                try:
                    ...
                    if code_blocks:
                        ...
                        content = last_block.inner_text()
                        
                        # 独自の終了マークがあるか、特定のフォーマットか確認
                        if "# filename:" in content and "# ---ここまで---" in content:
                            if content != self.last_sync_content:
                                self.last_sync_content = content
                                self.update_code_from_sync(content)
                                self.log("New code block detected and imported!", "success")
                except Exception as e:
                    self.log(f"Sync Watcher Warning: {e}", "info")
                
                time.sleep(3) # 負荷軽減のため3秒おきに監視
    ...
    def copy_log_to_clipboard(self):
        ...
        if log_content:
            ...
            self.log("Log content copied to clipboard.", "info")
