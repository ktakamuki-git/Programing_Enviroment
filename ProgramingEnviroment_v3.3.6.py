# filename: ProgramingEnviroment_v3.3.6.py
# --- ここから ---
...
class ProgramingEnviromentApp:
    ...
    def run_gemini_monitor(self):
        try:
            ...
            with sync_playwright() as p:
                # チャンネル指定によりEXE化後もシステムのChromeを利用
                    ...
                    if blocks:
                        ...
                        if match:
                            ...
                            if content != self.last_sync_content:
                                ...
                                self.root.after(0, lambda c=content: self.update_code(c))
                    time.sleep(2)
    ...
    def auto_extract_filename(self, event=None):
        ...
        if match:
            self.file_name_entry.delete(0, tk.END); self.file_name_entry.insert(0, match.group(1))
    ...
    def run_pipeline(self):
        ...
        is_sub = "sub_process: True" in code
        ...
        try:
            ...
            self.execute_git(repo_dir, os.path.join(target_dir, file_name))
            
            if not is_sub:
                self.execute_build(target_dir, file_name)
                # ビルド後に古いEXEをVersionsフォルダへ
        ...
        finally: self.action_btn.config(state="normal")
    ...
    def execute_build(self, target_dir, file_name):
        ...
        subprocess.run(["py", f"-{TARGET_PYTHON}", "-m", "PyInstaller", "--onefile", "--noconsole", f"--distpath={dist}", file_name], cwd=target_dir, shell=True)

    def cleanup_old_exes(self, target_dir=None, current_file=None):
        # 整理対象ディレクトリの決定（ツール自身か、プロジェクトのdistか）
        search_dir = os.path.join(target_dir, "dist") if target_dir else os.getcwd()
        if not os.path.exists(search_dir): return
        
        ver_dir = os.path.join(search_dir, "Versions")
        ...
        for f in os.listdir(search_dir):
            if f.endswith(".exe") and f != current_exe and f != "Versions":
                try:
                    shutil.move(os.path.join(search_dir, f), os.path.join(ver_dir, f))
                    self.log(f"Archived old EXE: {f}")
                except: pass
