# filename: ProgramingEnviroment_v3.3.0.py
...
class ProgramingEnviromentApp:
    ...
    def run_pipeline(self):
        ...
        if is_sub and main_parent:
            ...
            self.log(f"Sub-process detected. Targeting Main Parent: {prog_name}", "sync")
        else:
        ...
        try:
            ...
            # サブプロセスの場合は古いファイルの移動ルールを少し変えるか検討（今回はシンプルに維持）
    ...
    def organize_old_files(self, target_dir, current_file_name, prog_name):
        ...
        for f in os.listdir(target_dir):
            if f.endswith((".py", ".spec")) and f != current_file_name:
                ...
                # メインプロセス名のファイル、または同一名称の古いバージョンを移動
                if f.startswith(prog_name) or f.split('_v')[0] == current_file_name.split('_v')[0]:
