# filename: ProgramingEnviroment_v3.3.13.py
# sub_process: False
# --- ここから ---
...
class ProgramingEnviromentApp:
    ...
    def setup_ui(self):
        ...
        self.file_name_entry.pack(fill="x", pady=(0, 5))
        self.file_name_entry.insert(0, f"{APP_NAME}_v{VERSION}.py")

        self.stats_label = tk.Label(main_frame, text="Lines: 0 | Chars: 0", bg="#f8f9fa", fg="#5f6368", font=("Consolas", 9))
        ...
        self.code_area.bind("<KeyRelease>", self.on_editor_change)

        # 2種類のボタン配置
        btn_frame = tk.Frame(main_frame, bg="#f8f9fa")
        btn_frame.pack(fill="x", pady=(0, 10))

        self.run_btn = tk.Button(btn_frame, text="🚀 Save & Run (py)", command=lambda: self.start_process(mode="run"), 
                                bg="#34a853", fg="white", font=("Segoe UI", 11, "bold"), pady=8, bd=0)
        self.run_btn.pack(side="left", fill="x", expand=True, padx=(0, 5))

        self.build_btn = tk.Button(btn_frame, text="📦 Save, Push & Build EXE", command=lambda: self.start_process(mode="build"), 
                                  bg="#1a73e8", fg="white", font=("Segoe UI", 11, "bold"), pady=8, bd=0)
        self.build_btn.pack(side="left", fill="x", expand=True, padx=(5, 0))
    ...
    def update_stats(self):
        content = self.code_area.get("1.0", tk.END).rstrip()
        ...
        try:
            ...
            with sync_playwright() as p:
                self.browser_context = p.chromium.launch_persistent_context(
                    user_data_dir=session_dir, headless=False, channel="chrome", no_viewport=True,
    ...
    def update_code(self, content):
        ...
        self.on_editor_change(); self.root.bell()
    ...
    def get_project_name(self, file_name):
        if "_v" in file_name: return file_name.split("_v")[0]
        return re.split(r'[ \-]', file_name)[0] or APP_NAME

    def start_process(self, mode="run"):
        code = self.code_area.get("1.0", tk.END).strip()
        if "# ---ここまで---" not in code:
            messagebox.showerror("Error", "Footer tag '# ---ここまで---' is missing!")
            return
        
        self.run_btn.config(state="disabled")
        self.build_btn.config(state="disabled")
        threading.Thread(target=self.run_pipeline, args=(mode,), daemon=True).start()
    ...
    def run_pipeline(self, mode):
        ...
        try:
            ...
            if mode == "run":
                bat_path = os.path.join(target_dir, f"run_{file_name.replace('.py', '')}.bat")
                with open(bat_path, "w", encoding="shift-jis") as f:
                    f.write(f"@echo off\ntitle {file_name}\npy -{TARGET_PYTHON} {file_name}\npause")
                self.log(f"Batch Created: {os.path.basename(bat_path)}")
                subprocess.Popen(f'start cmd /c "{bat_path}"', shell=True, cwd=target_dir)
                self.log("Script launched via batch.", "success")
            elif mode == "build" and not is_sub:
                self.log(f"Building EXE for {file_name}...")
                self.execute_build(target_dir, file_name)
                self.cleanup_old_exes(target_dir, file_name)
                self.log("Build success.", "success")
            
            if mode == "build": messagebox.showinfo("Success", "Build Finished!")
    ...
    def cleanup_old_exes(self, target_dir=None, current_file=None):
        ...
        for f in os.listdir(base_dir):
            if f.endswith(".exe") and f != current_exe and f != "Versions":
                try: shutil.move(os.path.join(base_dir, f), os.path.join(ver_dir, f))
                except: pass
